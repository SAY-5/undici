...
class BodyReadable extends Readable {
  ...
  // setEncoding is inherited from node:stream's Readable, which installs a
  // StringDecoder and buffers partial multi-byte sequences between chunks.
  // Previously this class shadowed it with `_readableState.encoding = encoding`
  // only (no decoder), which made the base class fall back to
  // Buffer.prototype.toString(encoding) per chunk and replaced any UTF-8
  // multi-byte sequence that straddled a chunk boundary with U+FFFD (#5002).
  // Removing the override is the fix.
}
